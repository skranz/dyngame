# A holdup model with a seller buyer model
...
# If accepted, the seller can choose different effort levels from a finite grid between
# [0,1]

# Buyers  gain is  w0+e
# Sellers cost are k0+(1/2)*e^2

# We will later allow the buyer to interact with an outside option, which will be costly
# to create...
...
# Allow outside option to change...

make.funs = function(p.grid,e.grid,k0,w0,w.out,F.out) {
  ...
  act.fun = function(xv,m=NULL) {
    ...
    # Buyer faces offer of p and can accept or reject
      ...
      val = list(0,c(0,1))
      lab = list("",c("A","R"))
    # Seller can decide on effort level
  	} else if (xv == -3) {
    	val = list(e.grid,0)
    	lab = list(paste("e",0:(NROW(e.grid)-1),sep=""),"")
    	
    # Stage 3 after offer has been rejected: Buyer can decide to contract with
    # outside firm and thereby generate an outside option. This involves fixed cost
    } else if (xv == -4) {
    	val = list(0,c(0,1))
    	lab = list("",c("NO","Out"))
  	} else {
  ...
  g.fun = function(avm,xvm,m=NULL) {
    ...
    # Seller produces with effort e = avm[,1]
    rows = (xvm[,1] == -3)
    g[rows,1] = -k0 - 0.5*avm[rows,1]^2 
    g[rows,2] = w0  + avm[rows,1]

    # Contract has been rejected; Buyer has outside option
    rows = (xvm[,1] == -4 & xvm[,2]==1)
    g[rows,2] = w.out

    # Contract has been rejected; Buyer can generate an outside option
    rows = (xvm[,1] == -4 & xvm[,2]==0)
    g[rows,2] = - avm[rows,1] * F.out # Fixed cost of generating an outside option
  ...
  tau.fun = function(avm,xv,m=NULL) {
    store.local.objects("tau.fun")
  	...
  	rows = which(xv[,1] == -1)
  	if (length(rows)>0) {
   	  x.dest = xv.to.x(m,cbind(avm[rows,1],xvm[,2]))
  	  tau[cbind(rows,x.dest)] = 1
	  }
	  
  	# From any offered price we move to state with xv = -3 if accepted
  	rows = which((xv[,1] >= 0) & avm[,2] == 0)
  	if (length(rows)>0) {
    	x.dest = xv.to.x(m,cbind(-3,xvm[rows,2]))
    	tau[cbind(rows,x.dest)] = 1
    }  
    	
  	# From any offered price we move to state xv=-4 if rejected
  	rows = which((xv[,1] >= 0) & avm[,2] == 1)
  	if (length(rows)>0) {
    	x.dest = xv.to.x(m,cbind(-4,xvm[rows,2]))
      tau[cbind(rows,x.dest)] = 1
	  }
	  
  	# From state xv=-3 we always move to xv=-1
  	rows = which((xv[,1] == -3))
  	if (length(rows)>0) {
    	x.dest = xv.to.x(m,cbind(-1,xvm[rows,2]))
    	tau[cbind(rows,x.dest)] = 1
    }
  	# From state xv=-4 we always move to xv[,1]=-1
  	# Furthermore, player 1 can choose to invest in the outside option or not
  	rows = which((xv[,1] == -4 & (avm[,2] == 1 | xvm[,2]==1)))
  	if (length(rows)>0) {
  	  x.dest = xv.to.x(m,cbind(-1,1))
  	  tau[rows,x.dest] = 1
    }
...
e.grid = 1-(1/2)^(0:(ne-1))
ne = np = 101
e.grid = seq(0,1,length=ne)
p.grid = seq(k0,wH,length=np) # Offered prices can lie between kL and wH
funs = make.funs(p.grid=p.grid,e.grid=e.grid,k0=k0,w0=w0,w.out=w.out,F.out=F.out) 

xv.val = list(c(-1,-3,-4,p.grid),c(0,1))
xv.lab = list(c("St1","St3A","St3R",paste("p",round(p.grid,2),sep="")),c("NO","Out"))

m = init.game(n=2, name="Buyer/Seller", symmetric = FALSE,nxv=2,
