# Two countries can pollute the air 

make.funs = function(x.max,a.max, dep=0) {
  ...
  act.fun = function(xv,m=NULL) {
    store.local.objects("stat.act.fun")
  	...
  	val = list(0:min(x.max-xv,a.max),0:min(x.max-xv,a.max))
  	lab = val  	   
  ...
  g.fun = function(avm,xvm,m=NULL) {
    store.local.objects("g.fun");
  	...
  	catchrate = pmax(1,xvm/colSums(avm))
  	fish = avm *
...
K1 =  15   # Ressource endowments of the different players
K2 =  15  
q.max = 10
p.fun = function(Q) q.max-Q

funs = make.funs(K1=K1,K2=K2,q.max= q.max,p.fun = p.fun) 

xv.val = list(0:K1,0:K2)
